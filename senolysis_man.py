...
def main():
    ...
    if num_images > 9:
        ...
        with tqdm_joblib(tqdm(desc="Progress", total=len(img_paths))) as progress_bar:
            Parallel(n_jobs=cpus_to_use)(delayed(senolysis_analysis)(img_path,program_start_time) for img_path in img_paths)

    else:
        print(f'Number of Images less than 10, running analysis in series.')
        [senolysis_analysis(img_path,program_start_time) for img_path in tqdm(img_paths)]
